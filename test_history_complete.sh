...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}History Section - Complete Unit Tests${NC}"
echo -e "${BLUE}========================================${NC}\n"
...
# Test 1.1: GET /api/about/history
echo -e "\n${YELLOW}Test 1.1: GET /api/about/history${NC}"
HISTORY=$(curl -s "$BASE_URL/about/history")
test_endpoint "History endpoint accessible" "$HISTORY" '.success == true'
test_endpoint "Has section object" "$HISTORY" '.data.section != null'
test_endpoint "Has items array" "$HISTORY" '.data.items | type == "array"'
test_endpoint "Section has title" "$HISTORY" '.data.section.title != null'
test_endpoint "Section has subtitle" "$HISTORY" '.data.section.subtitle != null'
test_endpoint "Section has image_url field" "$HISTORY" '.data.section | has("image_url")'
test_endpoint "Items array not empty" "$HISTORY" '.data.items | length > 0'
test_endpoint "Items have required fields" "$HISTORY" '.data.items[0] | has("year") and has("title") and has("content")'
...
echo -e "\n${YELLOW}Test 2.1: GET /api/admin/about/history-section${NC}"
SECTION=$(curl -s -H "Authorization: Bearer $TOKEN" \
  "$BASE_URL/admin/about/history-section")
...
echo -e "\n${YELLOW}Test 2.2: PUT /api/admin/about/history-section${NC}"
UPDATE_SECTION=$(curl -s -X PUT "$BASE_URL/admin/about/history-section" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Test History Section",
    "subtitle": "Test subtitle for unit testing",
    "image_url": "/uploads/test/history-hero.jpg"
  }')
test_endpoint "Update section settings" "$UPDATE_SECTION" '.success == true'
test_endpoint "Title updated" "$UPDATE_SECTION" '.data.title == "Test History Section"'
test_endpoint "Subtitle updated" "$UPDATE_SECTION" '.data.subtitle == "Test subtitle for unit testing"'
test_endpoint "Image URL updated" "$UPDATE_SECTION" '.data.image_url == "/uploads/test/history-hero.jpg"'

# Test 2.3: Verify changes in public endpoint
echo -e "\n${YELLOW}Test 2.3: Verify section changes in public endpoint${NC}"
VERIFY=$(curl -s "$BASE_URL/about/history")
test_endpoint "Public endpoint reflects changes" "$VERIFY" '.data.section.title == "Test History Section"'
test_endpoint "Subtitle reflected" "$VERIFY" '.data.section.subtitle == "Test subtitle for unit testing"'
test_endpoint "Image URL reflected" "$VERIFY" '.data.section.image_url == "/uploads/test/history-hero.jpg"'

# ============================================
# ADMIN - HISTORY ITEMS TESTS
# ============================================
echo -e "\n${BLUE}3. Admin - History Items CRUD${NC}"

# Test 3.1: GET all items
echo -e "\n${YELLOW}Test 3.1: GET /api/admin/about/history${NC}"
ITEMS=$(curl -s -H "Authorization: Bearer $TOKEN" \
  "$BASE_URL/admin/about/history")
...
echo -e "\n${YELLOW}Test 3.2: POST /api/admin/about/history${NC}"
CREATE_ITEM=$(curl -s -X POST "$BASE_URL/admin/about/history" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "year": 2025,
    ...
    "image_url": "/uploads/history/test.jpg",
    "order_position": 99,
    "is_active": true
  }')
test_endpoint "Create item" "$CREATE_ITEM" '.success == true'
test_endpoint "Item has id" "$CREATE_ITEM" '.data.id != null'
test_endpoint "Item year correct" "$CREATE_ITEM" '.data.year == 2025'
test_endpoint "Item title correct" "$CREATE_ITEM" '.data.title == "Test Milestone"'
ITEM_ID=$(echo "$CREATE_ITEM" | jq -r '.data.id')

# Test 3.3: GET single item
echo -e "\n${YELLOW}Test 3.3: GET /api/admin/about/history/$ITEM_ID${NC}"
GET_ITEM=$(curl -s -H "Authorization: Bearer $TOKEN" \
  "$BASE_URL/admin/about/history/$ITEM_ID")
test_endpoint "Get single item" "$GET_ITEM" '.success == true'
test_endpoint "Item id matches" "$GET_ITEM" ".data.id == $ITEM_ID"
test_endpoint "Item has all fields" "$GET_ITEM" '.data | has("year") and has("title") and has("content")'

# Test 3.4: UPDATE item
echo -e "\n${YELLOW}Test 3.4: PUT /api/admin/about/history/$ITEM_ID${NC}"
UPDATE_ITEM=$(curl -s -X PUT "$BASE_URL/admin/about/history/$ITEM_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "year": 2026,
...
test_endpoint "Year updated" "$UPDATE_ITEM" '.data.year == 2026'
test_endpoint "Title updated" "$UPDATE_ITEM" '.data.title == "Updated Test Milestone"'
test_endpoint "Content updated" "$UPDATE_ITEM" '.data.content == "Updated test content"'

# Test 3.5: Verify item in public endpoint
echo -e "\n${YELLOW}Test 3.5: Verify item in public endpoint${NC}"
PUBLIC_ITEMS=$(curl -s "$BASE_URL/about/history")
test_endpoint "Item appears in public endpoint" "$PUBLIC_ITEMS" ".data.items | map(select(.id == $ITEM_ID)) | length > 0"

# Test 3.6: DELETE item
echo -e "\n${YELLOW}Test 3.6: DELETE /api/admin/about/history/$ITEM_ID${NC}"
DELETE_ITEM=$(curl -s -X DELETE "$BASE_URL/admin/about/history/$ITEM_ID" \
  -H "Authorization: Bearer $TOKEN")
...
STRUCTURE=$(curl -s "$BASE_URL/about/history")
...
test_endpoint "Item has year" "$STRUCTURE" '.data.items[0] | has("year")'
test_endpoint "Item has title" "$STRUCTURE" '.data.items[0] | has("title")'
test_endpoint "Item has content" "$STRUCTURE" '.data.items[0] | has("content")'
test_endpoint "Item has image_url" "$STRUCTURE" '.data.items[0] | has("image_url")'
...
NO_AUTH=$(curl -s "$BASE_URL/about/history")
...
NO_AUTH_ADMIN=$(curl -s "$BASE_URL/admin/about/history-section")
...
PARTIAL=$(curl -s -X PUT "$BASE_URL/admin/about/history-section" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
MINIMAL=$(curl -s -X POST "$BASE_URL/admin/about/history" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"year":2027,"title":"Minimal Item","content":"Minimal content"}')
test_endpoint "Create with minimal data" "$MINIMAL" '.success == true'
MINIMAL_ID=$(echo "$MINIMAL" | jq -r '.data.id')

# Cleanup minimal item
curl -s -X DELETE "$BASE_URL/admin/about/history/$MINIMAL_ID" \
...
curl -s -X PUT "$BASE_URL/admin/about/history-section" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Our History",
