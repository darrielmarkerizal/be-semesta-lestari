...
class Award extends BaseModel {
  ...
  async findAllPaginatedWithSearch(page = 1, limit = 10, isActive = null, search = null) {
    ...
    if (search) {
      conditions.push('(title LIKE ? OR short_description LIKE ? OR issuer LIKE ? OR year LIKE ?)');
    ...
    if (search) {
      countConditions.push('(title LIKE ? OR short_description LIKE ? OR issuer LIKE ? OR year LIKE ?)');
      const searchPattern = `%${search}%`;
